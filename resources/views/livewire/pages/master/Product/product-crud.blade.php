<div>
  ...
  <div class="w-full ">
    ...
    <form wire:submit="store" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      ...
      {{-- <x-select label="Country" placeholder="Country" wire:model="form.country" :async-data="$companies" /> --}}

      {{-- <x-select label="Country" placeholder="Country" wire:model="form.form" :options="$companies" /> --}}
      {{-- <pre>{{ print_r($companies, true) }}</pre>

      <x-select label="Country" placeholder="Select a country" wire:model="form.country" :options="$companies"
        option-value="value" option-label="label" /> --}}
      {{-- 
      <div class="mb-3">
        <x-label for="form.tes">Product Category 2</x-label>
        <x-select wire:model="form.tes" placeholder="Product Category 2" :async-data="$companies" option-label="name"
          option-value="id" />
      </div> --}}

      <pre>{{ print_r($form['country'], true) }}</pre>


      <div>
        <x-select label="Country" placeholder="Select a Country" wire:model="form.country" :async-data="$countries"
          option-label="label" option-value="value" />

        <!-- Show the selected country ID -->
        @if ($form['country'])
          <p>You selected the country with ID: {{ $form['country'] }}</p>
        @else
